...
class Environment(Base):
    ...
    def getAvailableEnvironmentById(envId, db):
        env = db.query(Environment).filter(Environment.id == envId, Environment.is_private == False).first() # noqa
        return env
    ...
    def getAllAvailableEnvironments(db):
        env = db.query(Environment).filter(Environment.is_private == False).all() # noqa
        return env

    @staticmethod
    def getAllAvailableEnvironmentsPaged(start_index, max_results, db):
        env = db.query(Environment).filter(Environment.is_private == False).limit(int(max_results)).offset(int(start_index)).all() # noqa
        return env

    @staticmethod
    def getAllAvailableEnvironmentsByType(type, db):
        env = db.query(Environment).filter(Environment.is_private == False, Environment.type == type).all() # noqa
        return env

    @staticmethod
    def getAllAvailableEnvironmentsByTypePaged(type, start_index, max_results, db):
        env = db.query(Environment).filter(Environment.is_private == False, Environment.type == type).limit(int(max_results)).offset(int(start_index)).all() # noqa
        return env

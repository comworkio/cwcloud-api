...
def admin_delete_user_2fa(current_user, userId, db):
    if not is_numeric(userId):
        return JSONResponse(content = {"error": "Invalid user id", "i18n_code": "400"}, status_code = 400)

    user = User.getUserById(userId, db)
    if not user:
        return JSONResponse(content = {"message" : "user not found", "i18n_code": "304"}, status_code = 404)
...
def admin_update_user(current_user, userId, payload, db):
    ...
    if is_not_email_valid(email):
        return JSONResponse(content = {"error": "the email is not valid", "i18n_code": "400"}, status_code = 400)
...
def admin_remove_user(current_user, userId, db):
    ...
    return JSONResponse(content = {"message" : "user successfully deleted", "i18n_code": "302"}, status_code = 200)
...
def admin_update_user_confirmation(current_user, userId, db):
    ...
    return JSONResponse(content = {"message" : "user successfully confirmed", "i18n_code": "303"}, status_code = 200)
...
def admin_add_user(current_user, payload, db):
    try:
        ...
        if is_not_email_valid(email):
            return JSONResponse(content = {"error": "the email is not valid", "i18n_code": "not_valid_email"}, status_code = 400)
        ...
        if is_flag_enabled(payload.enabled_features, 'disable_emails'):
            return JSONResponse(content = {"status": "ok", "message": "user successfully created", "i18n_code": "300"}, status_code = 200)
...
def admin_get_user(current_user, userId, db):
    ...
    return JSONResponse(content = {"result": userJson}, status_code = 200)

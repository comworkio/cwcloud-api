...
def admin_add_instance(current_user, payload, provider, region, zone, environment, db, bt: BackgroundTasks):
    ...
    if is_not_subdomain_valid(instance_name):
        return JSONResponse(content = {"error": "instance name is not valid", "i18n_code": "instance_name_invalid"}, status_code = 400)
    ...
    try:
        ...
        if is_empty(instance_name):
            return JSONResponse(content = {"error": "please provide instance name", "i18n_code": "1106"}, status_code = 400)
        if is_empty(email):
            return JSONResponse(content = {"error": "please provide an email", "i18n_code": "1102"}, status_code = 400)
        if is_empty(project_id) and is_empty(project_name) and is_empty(project_url):
            return JSONResponse(content = {"error": "please provide a project id or a project name or a project url", "i18n_code": "1107"}, status_code = 400)
        ...
        if  len(get_dns_zones())>0:
            ...
            if is_empty(root_dns_zone):
                ...
                return JSONResponse(content = {"error": "please provide a valid root dns zone", "i18n_code": "1108"}, status_code = 400)

        if is_empty(environment):
            return JSONResponse(content = {"error": "environment is missing", "i18n_code": "803"}, status_code = 400)
        if is_empty(instance_type):
            ...
            if instance_type not in possible_types:
                return JSONResponse(content = {"error": f"Instance type does not exist", "i18n_code": "105"}, status_code = 400)

        possible_regions = get_provider_infos(provider, "regions")
        if region not in [region["name"] for region in possible_regions]:
            return JSONResponse(content = {"error": f"region does not exist", "i18n_code": "1404"}, status_code = 400)
        if exists_zone(zone, region, possible_regions):
            return JSONResponse(content = {"error": "zone does not exist", "i18n_code": "1110"}, status_code = 400)

        from entities.Environment import Environment
        exist_env = Environment.getByPath(environment, db)
        if not exist_env:
            return JSONResponse(content = {"error": f"environment not found", "i18n_code": "804"}, status_code = 404)
        ...
        if is_not_project_found_in_gitlab(gitlab_project):
            return JSONResponse(content = {"error": f"project not found with gitlab", "i18n_code": "210"}, status_code = 404)
        ...
        if len(project_playbooks)>0:
            ...
            if instance_name in [name.split("playbook-")[1] for name in project_playbooks]:
                return JSONResponse(content = {"error": "playbook already exists", "i18n_code": "1111"}, status_code = 404)
        ...
        return JSONResponse(content = {"error": "stack already exists", "i18n_code": "1105"}, status_code = 409)
...
def admin_attach_instance(bt: BackgroundTasks, current_user, provider, region, zone, project_id, payload, db):
    ...
    if userInstance.status != "deleted":
        return JSONResponse(content = {"error": "Instance already running with this playbook", "i18n_code": "127"}, status_code = 404)
    ...
    if len(project_playbooks) == 0:
        return JSONResponse(content = {"error": "Project has no playbooks", "i18n_code": "206"}, status_code = 400)
    if is_empty(instance_name):
        return JSONResponse(content = {"error": "please specify the playbook you want to attach.", "i18n_code": "121"}, status_code = 400)

    if not instance_name in [name.split("playbook-")[1] for name in project_playbooks]:
        return JSONResponse(content = {"error": "playbook not found", "i18n_code": "122"}, status_code = 404)
...
def admin_get_instance(current_user, instance_id, db):
    if not is_numeric(instance_id):
        return JSONResponse(content = {"error": "Invalid instance id"}, status_code = 400)
...
def admin_update_instance(current_user, instance_id, payload, db):
    ...
    if not action in possible_actions:
        return JSONResponse(content = {"error": "action doesnt exist", "i18n_code": "106"}, status_code = 400)
    server_state = get_server_state(userInstance.provider, server)
    if action == "poweroff":
        if server_state == "stopped":
            return JSONResponse(content = {"error": "Instance already stopped", "i18n_code": "107"}, status_code = 400)
        if server_state == "rebooting":
            return JSONResponse(content = {"error": "You can\"t stop the Instance while rebooting", "i18n_code": "108"}, status_code = 400)
        if server_state == "starting":
            return JSONResponse(content = {"error": "You can\"t stop the Instance while starting", "i18n_code": "109"}, status_code = 400)
        if server_state == "stopping":
            return JSONResponse(content = {"error": "Instance already stopping", "i18n_code": "110"}, status_code = 400)
    if action == "poweron":
        if server_state == "running":
            return JSONResponse(content = {"error": "Instance already running", "i18n_code": "111"}, status_code = 400)
        if server_state == "rebooting":
            return JSONResponse(content = {"error": "You can\"t start the Instance while rebooting", "i18n_code": "112"}, status_code = 400)
        if server_state == "starting":
            return JSONResponse(content = {"error": "Instance already starting", "i18n_code": "113"}, status_code = 400)
        if server_state == "stopping":
            return JSONResponse(content = {"error": "You can\"t start the Instance while stopping", "i18n_code": "114"}, status_code = 400)
    if action == "reboot":
        if server_state == "stopped":
            return JSONResponse(content = {"error": "You can\"t reboot the Instance when it is stopped", "i18n_code": "115"}, status_code = 400)
        if server_state == "rebooting":
            return JSONResponse(content = {"error": "Instance already rebooting", "i18n_code": "116"}, status_code = 400)
        if server_state == "starting":
            return JSONResponse(content = {"error": "You can\"t reboot the Instance while starting", "i18n_code": "117"}, status_code = 400)
        if server_state == "stopping":
            return JSONResponse(content = {"error": "You can\"t reboot the Instance while stopping", "i18n_code": "118"}, status_code = 400)
    try:
        if userInstance.status == "active" and action == "activate":
            return JSONResponse(content = {"error": "instance already active", "i18n_code": "119"}, status_code = 400)

...
def get_bucket(current_user, provider, region, bucket_id, db):
    ...
    if not is_numeric(bucket_id):
        return JSONResponse(content = {"error": "Invalid bucket id", "i18n_code": "400"}, status_code = 400)
    ...
    if not user_bucket:
        ...
        if not access:
            return JSONResponse(content = {"error": "Bucket not found", "i18n_code": "404"}, status_code = 404)
...
def remove_bucket(current_user, provider, region, bucket_id, db, bt: BackgroundTasks):
    ...
    try:
        ...
        return JSONResponse(content = {"message": "bucket successfully deleted", "i18n_code": "402"}, status_code = 200)

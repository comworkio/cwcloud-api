...
def get_registry(current_user, provider, region, registryId, db):
    ...
    if not is_numeric(registryId):
        return JSONResponse(content = {"error": "Invalid registry id", "i18n_code": "400"}, status_code = 400)
...
def remove_registry(current_user, provider, region, registryId, db, bt: BackgroundTasks):
    ...
    try:
        ...
        return JSONResponse(content = {"message": "registry successfully deleted", "i18n_code": "902"}, status_code = 200)

...
def get_instance(current_user, provider, region, instance_id, db):
    ...
    if not is_numeric(instance_id):
        return JSONResponse(content = {"error": "Invalid instance id", "i18n_code": "400"}, status_code = 400)
...
def attach_instance(bt: BackgroundTasks, current_user, provider, region, zone, project_id, payload, db):
    ...
    if is_not_subdomain_valid(instance_name):
        return JSONResponse(content = {"error": "The instance name is not valid", "i18n_code": "instance_name_invalid"}, status_code = 400)
    ...
    if is_flag_disabled(user.enabled_features, 'billable') and not user.is_admin:
        return JSONResponse(content = {"error": "you dont have access to create resources", "i18n_code": "126"}, status_code = 400)
    if not exist_provider(provider):
        return JSONResponse(content = {"error": "provider does not exist", "i18n_code": "504"}, status_code = 404)

...
def cwaiapi_required(current_user: UserSchema = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if is_false(is_granted) and is_not_empty(current_user):
        ...
        is_granted = is_flag_enabled(user.enabled_features, 'cwaiapi')

    if is_false(is_granted):
        raise CwHTTPException(message = {"error": "permission denied", "i18n_code": "not_cwaiapi"}, status_code = status.HTTP_403_FORBIDDEN)
